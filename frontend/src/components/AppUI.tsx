...
const AppUI: React.FC = () => {
  ...
  const DashboardView = () => (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6 h-full">
      {/* Main Meeting Area */}
      <div className={`lg:col-span-2 p-4 lg:p-8 rounded-2xl lg:rounded-3xl ${glassPanel} hover:scale-[1.01] transition-transform duration-300`}>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 lg:mb-8">
          ...
          <div className="flex space-x-2 lg:space-x-3">
            <button className={`p-2 lg:p-3 rounded-lg lg:rounded-xl ${glassCard} hover:scale-110 transition-transform`}>
        ...
        <div className={`p-4 lg:p-6 rounded-xl lg:rounded-2xl mb-6 lg:mb-8 ${glassCard}`}>
          <div className="flex items-center space-x-3 lg:space-x-4 mb-4 lg:mb-6">
            ...
            <div>
              <h3 className={`text-lg lg:text-xl font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>
                AI Processing Live
              </h3>
          ...
          <div className="space-y-3 lg:space-y-4">
            {[
              ...
              { text: "Schedule follow-up meeting with clients", status: "completed", color: "emerald" },
              { text: "Update project timeline in ClickUp", status: "syncing", color: "blue" }
            ].map((item, index) => (
              <div key={index} className={`p-3 lg:p-4 rounded-lg lg:rounded-xl border-l-4 ${
                item.color === 'amber' ? 'border-amber-400 bg-amber-500/10' :
                item.color === 'emerald' ? 'border-emerald-400 bg-emerald-500/10' :
                'border-blue-400 bg-blue-500/10'
              }`}>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <p className={`text-xs lg:text-sm font-medium ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>
                    {item.text}
                  </p>
                  <span className={`text-xs px-2 py-1 rounded-full font-medium self-start sm:self-center ${
                    item.color === 'amber' ? 'text-amber-400 bg-amber-400/20' :
                    item.color === 'emerald' ? 'text-emerald-400 bg-emerald-400/20' :
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 lg:gap-6">
          {[
            { icon: Zap, value: '12', label: 'Tasks Created', change: '+5 today', trend: 'up' },
            { icon: Clock, value: '47m', label: 'Time Saved', change: 'This week', trend: 'up' },
            { icon: Target, value: '94%', label: 'Accuracy', change: 'AI precision', trend: 'stable' },
          ].map((stat, index) => (
            <div key={index} className={`p-4 lg:p-6 rounded-xl lg:rounded-2xl ${glassCard} hover:scale-105 transition-transform`}>
      ...
      <div className="space-y-4 lg:space-y-6">
        ...
        <div className={`p-4 lg:p-6 rounded-2xl lg:rounded-3xl ${glassPanel}`}>
          <h3 className={`text-lg lg:text-xl font-bold mb-4 lg:mb-6 ${isDark ? 'text-white' : 'text-slate-900'}`}>
            Quick Actions
          </h3>
          <div className="space-y-2 lg:space-y-3">
            {[
              ...
              <button key={index} className={`w-full flex items-center space-x-3 p-3 lg:p-4 rounded-xl lg:rounded-2xl ${glassCard} hover:scale-[1.02] transition-all`}>
                <action.icon className={`w-4 lg:w-5 h-4 lg:h-5 ${
        ...
        <div className={`p-4 lg:p-6 rounded-2xl lg:rounded-3xl ${glassPanel}`}>
          ...
          <div className="space-y-3 lg:space-y-4">
            {[
              { action: 'Created 3 tasks', time: '2 min ago', user: 'AI Assistant' },
              { action: 'Meeting ended', time: '15 min ago', user: 'Product Team' },
              { action: 'Synced to ClickUp', time: '1 hour ago', user: 'Integration' },
            ].map((item, index) => (
              <div key={index} className={`p-3 lg:p-4 rounded-xl lg:rounded-2xl ${glassCard}`}>
                <p className={`font-medium text-xs lg:text-sm ${isDark ? 'text-white' : 'text-slate-900'}`}>
                  {item.action}
                </p>
      ...
      <div className={`min-h-screen transition-all duration-300 ${
        ...
        <div className="relative z-10 flex h-screen overflow-hidden">
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            {/* Top Navigation Bar */}
            <div className={`m-3 lg:m-6 p-4 lg:p-6 backdrop-blur-3xl border rounded-2xl lg:rounded-3xl transition-all duration-500 ${glassPanel}`}>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h1 className={`text-xl lg:text-3xl font-bold ${
                      ...
                      {activeTab === 'dashboard' ? 'Good afternoon, Alex' :
                       activeTab === 'meetings' ? 'Live Meetings' :
                       activeTab === 'tasks' ? 'Action Items' :
                       activeTab === 'analytics' ? 'Analytics' :
                       activeTab === 'calendar' ? 'Calendar' :
                       'Settings'}
                    </h1>
                    <p className={`mt-1 text-sm lg:text-base ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                      {activeTab === 'dashboard' && `AI processing ${processingTasks} active conversations`}
