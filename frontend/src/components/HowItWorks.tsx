  ...
  const steps = [
    {
      ...
      description: 'Our AI bot joins automatically or use our desktop/mobile apps to capture any conversation.',
    ...
    },
    {
      ...
      icon: Cpu,
      title: 'AI EXTRACTION',
      description: 'Advanced neural networks analyze the conversation and identify action items, assignees, and deadlines.',
    ...
    },
    {
      ...
      icon: Sync,
      title: 'AUTO SYNC',
      description: 'Tasks are instantly created in your preferred project management platform with all relevant context.',
    ...
    },
    {
      ...
      description: 'Focus on execution instead of manual task creation. Your team stays aligned and productive.',
  ...
  return (
    <section id="how-it-works" className={`py-24 relative overflow-hidden transition-colors duration-300 ${
      ...
      <div className="absolute inset-0">
        <div className={`absolute top-1/4 right-1/4 w-72 h-72 rounded-full filter blur-3xl animate-pulse ${
          isDark ? 'bg-cyan-500/10' : 'bg-cyan-500/20'
        }`}></div>
        <div className={`absolute bottom-1/4 left-1/4 w-72 h-72 rounded-full filter blur-3xl animate-pulse delay-1000 ${
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 md:gap-8 px-4">
            {steps.map((step, index) => (
              ...
              >
                ...
                <div className={`relative p-6 md:p-8 rounded-2xl border transition-all duration-300 hover:scale-105 text-center ${
                  isDark 
                  ...
                  {/* Glow Effect */}
                  <div className={`absolute -inset-0.5 bg-gradient-to-r rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 blur ${
                    isDark ? 'from-cyan-500/10 to-purple-500/10' : 'from-cyan-500/20 to-purple-500/20'
                  }`}></div>
                  
                  <div className="relative">
                    {/* Step Number */}
                    <div className={`text-4xl md:text-6xl font-bold bg-gradient-to-r bg-clip-text text-transparent mb-4 ${
                    ...
                    {/* Icon */}
                    <div className={`w-16 md:w-20 h-16 md:h-20 bg-gradient-to-r ${step.gradient} rounded-2xl flex items-center justify-center mb-4 md:mb-6 mx-auto group-hover:scale-110 transition-transform duration-300`}>
                      <step.icon className="w-8 md:w-10 h-8 md:h-10 text-white" />
                    </div>

                    {/* Content */}
                    <h3 className={`text-lg md:text-xl font-bold mb-3 md:mb-4 tracking-wide ${
