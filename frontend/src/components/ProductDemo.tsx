...
const sampleConversations = {
  agency_call: {
    ...
    duration: '22:15',
    date: '2025-01-15',
    participants: ['Sarah (Account Manager)', 'Mike (Creative Director)', 'Jennifer (Brand Manager - Client)'],
...
Mike: Definitely. I'll also prepare three alternative layouts based on the new guidelines.`,
    actionItems: [
      { id: 1, task: 'Send updated brand guidelines', assignee: 'Jennifer (Client)', priority: 'High', dueDate: '2025-01-16', completed: false },
      { id: 2, task: 'Adjust homepage color palette', assignee: 'Mike', priority: 'High', dueDate: '2025-01-17', completed: false },
      { id: 3, task: 'Update proposal with $15K budget increase', assignee: 'Sarah', priority: 'Medium', dueDate: '2025-01-16', completed: false },
      { id: 4, task: 'Schedule follow-up meeting for Friday', assignee: 'Sarah', priority: 'Medium', dueDate: '2025-01-15', completed: true },
      { id: 5, task: 'Prepare three alternative layouts', assignee: 'Mike', priority: 'Medium', dueDate: '2025-01-17', completed: false },
      { id: 6, task: 'Review social media budget allocation', assignee: 'Sarah', priority: 'Low', dueDate: '2025-01-18', completed: false }
    ],
    decisions: [
  ...
  consultant_call: {
    ...
    actionItems: [
      { id: 1, task: 'Implement new inventory management system', assignee: 'Rebecca', priority: 'High', dueDate: '2025-02-01', completed: false },
      { id: 2, task: 'Hire additional operations staff', assignee: 'Tom', priority: 'High', dueDate: '2025-01-25', completed: false },
      { id: 3, task: 'Create training materials for new system', assignee: 'David', priority: 'Medium', dueDate: '2025-01-28', completed: false },
      { id: 4, task: 'Schedule system deployment meeting', assignee: 'David', priority: 'Medium', dueDate: '2025-01-20', completed: false }
    ],
    decisions: [
  ...
  msp_call: {
    ...
    actionItems: [
      { id: 1, task: 'Update SLA response times to 2 hours', assignee: 'John', priority: 'High', dueDate: '2025-01-18', completed: false },
      { id: 2, task: 'Install additional monitoring tools', assignee: 'Alex', priority: 'High', dueDate: '2025-01-22', completed: false },
      { id: 3, task: 'Provide monthly security reports', assignee: 'Alex', priority: 'Medium', dueDate: '2025-02-01', completed: false },
      { id: 4, task: 'Schedule quarterly business review', assignee: 'John', priority: 'Low', dueDate: '2025-01-30', completed: false }
    ],
    decisions: [
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                        key === 'agency_call' ? 'bg-blue-500' :
                        key === 'consultant_call' ? 'bg-purple-500' : 'bg-emerald-500'
                      }`}>
                        {key === 'agency_call' ? <FileText className="w-6 h-6 text-white" /> :
                         key === 'consultant_call' ? <BarChart3 className="w-6 h-6 text-white" /> :
          ...
          <div className="max-w-7xl mx-auto">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    <span className={isDark ? 'text-slate-400' : 'text-slate-600'}>
                      {currentConversation.duration} • {currentConversation.participants.length} participants
                    </span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              <div className={`p-6 rounded-2xl ${glassCard} text-center`}>
                <div className="text-3xl font-bold text-blue-500 mb-2">{currentConversation.actionItems.length}</div>
                <div className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>Action Items</div>
              </div>
              <div className={`p-6 rounded-2xl ${glassCard} text-center`}>
                <div className="text-3xl font-bold text-purple-500 mb-2">{currentConversation.decisions.length}</div>
                <div className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>Key Decisions</div>
              </div>
              <div className={`p-6 rounded-2xl ${glassCard} text-center`}>
                <div className="text-3xl font-bold text-emerald-500 mb-2">{currentConversation.urgentItems}</div>
                <div className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>Urgent Items</div>
              </div>
              <div className={`p-6 rounded-2xl ${glassCard} text-center`}>
                <div className="text-3xl font-bold text-orange-500 mb-2">{currentConversation.satisfactionScore}</div>
                <div className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>Satisfaction</div>
              </div>
            </div>
                    ...
                    <div className="mb-8">
                      ...
                      <div className="space-y-3">
                        {currentConversation.decisions.map((decision, index) => (
                          <div key={index} className={`p-4 rounded-xl ${glassCard} flex items-start`}>
                            <CheckCircle2 className="w-5 h-5 text-green-400 mr-3 mt-0.5 flex-shrink-0" />
                            <span className={isDark ? 'text-slate-300' : 'text-slate-700'}>{decision}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-3">
                        {currentConversation.participants.map((participant, index) => (
                          <div key={index} className={`px-4 py-2 rounded-xl ${glassCard} flex items-center`}>
                  ...
                  <div className={`p-8 rounded-3xl ${glassPanel}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {[
                        ...
                        { name: 'ClickUp', color: 'from-purple-500 to-pink-500', tasks: 6 },
                        { name: 'Monday.com', color: 'from-blue-500 to-cyan-500', tasks: 6 },
                        { name: 'Slack', color: 'from-green-500 to-emerald-500', notifications: 8 }
                      ].map((integration) => (
                        <div key={integration.name} className={`p-6 rounded-2xl ${glassCard} hover:scale-105 transition-transform`}>
