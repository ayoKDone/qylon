...
export class MeetingService {
  ...
  async getMeetings(page: number = 1, limit: number = 10): Promise<ApiResponse<PaginatedResponse<Meeting>>> {
  ...
  async createMeeting(meeting: Omit<Meeting, 'id' | 'created_at' | 'updated_at'>): Promise<ApiResponse<Meeting>> {
  ...
  async createActionItem(meetingId: string, actionItem: Omit<ActionItem, 'id' | 'meeting_id' | 'created_at' | 'updated_at'>): Promise<ApiResponse<ActionItem>> {
    return apiService.post<ActionItem>(`${this.baseEndpoint}/${meetingId}/action-items`, actionItem);
  }

  async updateActionItem(meetingId: string, actionItemId: string, updates: Partial<ActionItem>): Promise<ApiResponse<ActionItem>> {
    return apiService.put<ActionItem>(`${this.baseEndpoint}/${meetingId}/action-items/${actionItemId}`, updates);
  }

  async deleteActionItem(meetingId: string, actionItemId: string): Promise<ApiResponse<void>> {
    return apiService.delete<void>(`${this.baseEndpoint}/${meetingId}/action-items/${actionItemId}`);
  }
