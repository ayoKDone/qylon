...
test('Event model structure', () => {
    try {
        ...
        const requiredFields = ['id', 'aggregateId', 'aggregateType', 'eventType', 'eventData', 'eventVersion', 'timestamp', 'userId'];
...
test('Saga model structure', () => {
    try {
        ...
        // Validate required fields
        const requiredFields = ['id', 'name', 'status', 'steps', 'currentStepIndex', 'correlationId', 'userId', 'startedAt'];
        for (const field of requiredFields) {
            if (!(field in saga)) {
