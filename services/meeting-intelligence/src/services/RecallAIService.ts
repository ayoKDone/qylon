...
export class RecallAIService {
   ...
   */
  async createBot(meetingUrl: string, botName?: string, options?: {
    clientId?: string;
    teamId?: string;
    autoStart?: boolean;
  ...
  variant?: {
    ...
    try {
      ...
      const isScheduled = options?.joinAt &&
      ...
      const botConfig = {
        ...
        recording_config: {
          transcript: {
            provider: {
              [options?.transcriptionProvider || 'meeting_captions']: options?.transcriptionProvider === 'recallai_streaming' ? {
          ...
          video_mixed_participant_video_when_screenshare: options?.recordingConfig?.videoMixedParticipantVideoWhenScreenshare || 'overlap',
        ...
        ...(options?.automaticLeave && {
          automatic_leave: {
              ...
              bot_detection: {
                ...(options.automaticLeave.botDetection.usingParticipantEvents && {
                  using_participant_events: {
                    timeout: options.automaticLeave.botDetection.usingParticipantEvents.timeout || 600,
                    activate_after: options.automaticLeave.botDetection.usingParticipantEvents.activateAfter || 1200
                  }
                }),
                ...(options.automaticLeave.botDetection.usingParticipantNames && {
                  using_participant_names: {
                    matches: options.automaticLeave.botDetection.usingParticipantNames.matches || ['bot', 'notetaker', 'qylon'],
                    timeout: options.automaticLeave.botDetection.usingParticipantNames.timeout || 600,
                    activate_after: options.automaticLeave.botDetection.usingParticipantNames.activateAfter || 1200
                  }
                })
              }
            }),
            ...
            recording_permission_denied_timeout: options.automaticLeave.recordingPermissionDeniedTimeout || 30
          }
        }),
        ...(options?.webhookUrl && {
   ...
   */
  async getBotScreenshots(botId: string): Promise<Array<{
    id: string;
    bot_id: string;
   ...
   */
  async getBotScreenshot(botId: string, screenshotId: string): Promise<{
    id: string;
    bot_id: string;
    screenshot_url: string;
    created_at: string;
  ...
  private analyzeFatalError(statusChange: any): {
    ...
    const recommendations: Record<string, string> = {
      ...
      'meeting_not_found': 'No meeting was found at the given link. Verify the meeting URL is correct.',
      'meeting_not_started': 'The meeting has not started yet. Wait for the meeting to begin.',
      'meeting_requires_registration': 'The meeting requires registration. Ensure bot is joining from an allowed webinar URL.',
      'meeting_requires_sign_in': 'The meeting requires sign-in. Configure authenticated bot credentials.',
      ...
      'zoom_sdk_credentials_missing': 'Zoom SDK credentials are not configured. Set up Zoom credentials in Recall dashboard.',
      ...
      'zoom_email_blocked_by_admin': 'The Zoom account has been disallowed by the workspace administrator.',
      ...
      'zoom_account_blocked': 'The account has been blocked by Zoom. Generate ZAK token from a different account.',
      'zoom_invalid_join_token': 'Zoom SDK rejected the join token. Check token validity and permissions.',
      ...
      'zoom_web_disallowed': 'Host has disallowed joining from web. Disable E2E encryption or use Native bot.',
      'zoom_connection_failed': 'Failed to join due to Zoom server error.',
      'zoom_error_multiple_device_join': 'Another participant with same credentials joined the meeting.',
      ...
      'google_meet_sign_in_missing_login_credentials': 'Login credentials not configured for authenticated bot.',
      ...
      'microsoft_teams_event_not_started_for_external': 'External participants cannot join before event begins.',
  ...
  private analyzeWarningStatus(statusChange: any): {
    ...
    const recommendations: Record<string, string> = {
      ...
      'in_call_not_recording': 'The bot joined the call but is not recording. Check recording permissions.',
  ...
  private analyzeCallEndedStatus(statusChange: any): {
    ...
    const recommendations: Record<string, string> = {
      ...
      'call_ended_by_platform_max_length': 'The call reached the maximum meeting length (e.g., 40 minutes on free Zoom).',
      'call_ended_by_platform_waiting_room_timeout': 'Bot could not join due to platform waiting room timeout (Google Meet: 10 minutes).',
      'timeout_exceeded_waiting_room': 'Bot left due to waiting room timeout (automatic_leave.waiting_room_timeout).',
      ...
      'timeout_exceeded_only_bots_detected_using_participant_names': 'Bot left because only bots detected by name patterns.',
      'timeout_exceeded_only_bots_detected_using_participant_events': 'Bot left because only bots detected by activity patterns.',
      ...
      'timeout_exceeded_only_bots_in_call': 'Bot left because only bot participants remained in the call.'
    };
  ...
  async createBotForScheduledMeeting(
    ...
    try {
      ...
      const isScheduled = scheduledTime &&
        (scheduledTime.getTime() - Date.now()) >= (10 * 60 * 1000);
   ...
   */
  async getSeparateVideos(recordingId: string): Promise<Array<{
    id: string;
    recording: {
      id: string;
      metadata: any;
   ...
   */
  async getSeparateVideoParts(separateVideoData: any): Promise<Array<{
    participant: {
      id: number;
      name: string | null;
      is_host: boolean;
   ...
   */
  async getSeparateAudios(recordingId: string): Promise<Array<{
    id: string;
    recording: {
      id: string;
      metadata: any;
   ...
   */
  async getSeparateAudioParts(separateAudioData: any): Promise<Array<{
    participant: {
      id: number;
      name: string | null;
      is_host: boolean;
   ...
   */
  async createAsyncTranscript(recordingId: string, provider: any, diarization?: any): Promise<{
    id: string;
    recording: {
      id: string;
      metadata: any;
    ...
    try {
      ...
      const response = await this.client.post(`/recording/${recordingId}/create_transcript`, requestBody);
   ...
   */
  async downloadTranscript(downloadUrl: string): Promise<Array<{
    participant: {
      id: number;
      name: string | null;
      is_host: boolean;
