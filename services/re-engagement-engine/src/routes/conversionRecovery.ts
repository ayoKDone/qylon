...
import { CreateRecoveryCampaignRequest, RecoveryResult, UpdateRecoveryCampaignRequest } from '../types';
...
const createRecoveryCampaignSchema = Joi.object({
  ...
  recoveryStrategy: Joi.string().valid(
    'email_sequence',
...
router.put('/campaigns/:campaignId', asyncHandler(async (req: Request, res: Response) => {
  ...
  const campaign = await conversionRecoveryService.updateRecoveryCampaign(campaignId, userId, request);
 ...
 */
router.post('/campaigns/:campaignId/execute', asyncHandler(async (req: Request, res: Response) => {
  const userId = req.user?.id;
  if (!userId) {
    throw createError('User not authenticated', 401, 'UNAUTHORIZED');
  }
 ...
 */
router.get('/campaigns/:campaignId/executions', asyncHandler(async (req: Request, res: Response) => {
  const userId = req.user?.id;
  if (!userId) {
    throw createError('User not authenticated', 401, 'UNAUTHORIZED');
  }
 ...
 */
router.post('/executions/:executionId/complete', asyncHandler(async (req: Request, res: Response) => {
  const userId = req.user?.id;
  if (!userId) {
    throw createError('User not authenticated', 401, 'UNAUTHORIZED');
  }
