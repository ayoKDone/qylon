...
Cypress.Commands.add(
  'shouldHaveFormErrors',
  (formTestId: string, expectedErrors: string[]) => {
...
Cypress.Commands.add(
  'shouldHaveTableData',
  (tableTestId: string, expectedData: any[]) => {
    cy.get(`[data-testid="${tableTestId}"] tbody tr`).should(
      'have.length',
      expectedData.length
    );
    ...
    expectedData.forEach((rowData, index) => {
      cy.get(
        `[data-testid="${tableTestId}"] tbody tr:nth-child(${index + 1})`
      ).within(() => {
...
Cypress.Commands.add('shouldHaveEmptyTable', (tableTestId: string) => {
  ...
  cy.get(`[data-testid="${tableTestId}"] [data-testid="no-data"]`).should(
    'be.visible'
  );
});
...
Cypress.Commands.add(
  'shouldHaveFieldValue',
  (fieldTestId: string, expectedValue: string) => {
    cy.get(`[data-testid="${fieldTestId}"]`).should(
      'have.value',
      expectedValue
    );
  }
...
Cypress.Commands.add(
  'shouldHaveFieldError',
  (fieldTestId: string, expectedError: string) => {
    cy.get(`[data-testid="${fieldTestId}"]`).should('have.class', 'error');
    cy.get(
      `[data-testid="${fieldTestId}"] + [data-testid="field-error"]`
...
Cypress.Commands.add(
  ...
  (selectTestId: string, expectedValue: string) => {
    cy.get(`[data-testid="${selectTestId}"]`).should(
      'have.value',
      expectedValue
    );
  }
...
Cypress.Commands.add(
  'shouldHaveUploadedFile',
  (inputTestId: string, fileName: string) => {
...
Cypress.Commands.add(
  ...
  (paginationTestId: string, currentPage: number, totalPages: number) => {
    ...
    cy.get(
      `[data-testid="${paginationTestId}"] [data-testid="current-page"]`
    ).should('contain', currentPage);
    cy.get(
      `[data-testid="${paginationTestId}"] [data-testid="total-pages"]`
    ).should('contain', totalPages);
  }
);
...
Cypress.Commands.add(
  ...
  (searchTestId: string, expectedCount: number) => {
    cy.get(
      `[data-testid="${searchTestId}"] [data-testid="search-results"]`
    ).should('have.length', expectedCount);
  }
);

Cypress.Commands.add('shouldHaveNoSearchResults', (searchTestId: string) => {
  cy.get(`[data-testid="${searchTestId}"] [data-testid="no-results"]`).should(
    'be.visible'
  );
});
...
Cypress.Commands.add(
  'shouldLoadWithinTime',
  (testId: string, maxTime: number) => {
